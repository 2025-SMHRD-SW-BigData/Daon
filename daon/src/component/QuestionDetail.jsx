...
const QuestionDetail = ({ user }) => {
    ...
    useEffect(() => {
        if (user === undefined) return; // user가 아직 설정되지 않은 경우 대기
        if (!user) {
        ...
        <div className="phon_size">
            ...
            <div className="qna-container">
                ...
                <div className="answer-list" style={{ marginTop: '30px' }}>
                    <h4 style={{ marginTop: '-10px', marginBottom: '10px' }}>📬 등록된 답변</h4>
                    {answers.length === 0 ? (
                        <p>아직 답변이 없습니다.</p>
                    ) : (
                        answers.map(ans => (
                            <div key={ans.answer_id} style={{
                                ...
                                <p style={{ whiteSpace: 'pre-wrap', marginTop: '10px' }}>{ans.answer}</p>
                                <p style={{ fontSize: '12px', color: '#999', textAlign: 'right', marginTop: '5px' }}>
